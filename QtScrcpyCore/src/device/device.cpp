...
{
    qDebug() << "Device::Device constructor, this: " << this << "serial: " << m_params.serial;
...
Device::~Device()
{
    qDebug() << "Device::~Device, this: " << this << "serial: " << m_params.serial;
...
quint16 Device::getLocalPort()
{
    if (m_server) {
        return m_server->getParams().localPort;
    }
    return 0;
}
...
void Device::initSignals()
{
    ...
    if (m_fileHandler) {
        connect(m_fileHandler, &FileHandler::fileHandlerResult, this, [this](FileHandler::FILE_HANDLER_RESULT processResult, bool isApk) {
            ...
            } else if (FileHandler::FAR_SUCCESS_EXEC == processResult) {
                if (isApk) {
                    tips = QString("%1 complete").arg(tipsType);
                } else {
                    tips = QString("%1 complete, save in %2").arg(tipsType).arg(m_params.pushFilePath);
                }
            } else if (FileHandler::FAR_ERROR_EXEC == processResult) {
