...
public:
    ...
    Q_SIGNAL void error(int status, QString errorString, QString result, QVariant userData);

    Q_SIGNAL void success(QString result, QVariant userData);

    Q_SIGNAL void cache(QString result, QVariant userData);

    Q_SIGNAL void uploadProgress(qint64 sent, qint64 total);

    Q_SIGNAL void downloadProgress(qint64 recv, qint64 total);

    Q_SIGNAL void chunck(QString data, QVariant userData);
};
...
public:
    ...
    Q_INVOKABLE NetworkParams *addList(const QString &key, const QVariantList &val);

    Q_INVOKABLE NetworkParams *addMap(const QString &key, const QVariantMap &val);
    ...
    Q_INVOKABLE NetworkParams *setUserData(const QVariant &data);
