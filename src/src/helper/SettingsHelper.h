...
public:
    ...
    Q_INVOKABLE QString getLanguage() {
        return get("language", QVariant(QLocale::system().name().split("_").front())).toString();
    }

    Q_INVOKABLE void save(const QString &key, const QVariant& val);
    Q_INVOKABLE QVariant get(const QString &key, const QVariant& def = {});
