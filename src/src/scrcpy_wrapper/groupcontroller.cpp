...
void GroupController::setRenderSink(const QString &serial, armcloud::VideoRenderSink *sink)
{
    if(m_renderSink.contains(serial)){
...
{
    qDebug() << "GroupController::mouseEvent" << from << frameSize << showSize;
    Q_UNUSED(frameSize);
